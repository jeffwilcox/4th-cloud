  ...
  function MongoStore(options, callback) {
    ...
    if (options.mongoose_connection){
      ...
      if (options.connectionString) {
        ...
        // Modern 1.3x drivers support connection string formats, including 
        // replicaset information and default parameters.
        this.modernDb = options.modernDb = true;
        MongoClient.connect(options.connectionString, driverOptions, function (err, db) {
          if (err) {
            throw new Error(err);            
          }
          this.db = db;
          if (db.collection) {
            self._set_collection_with_index(db.collection(self.db_collection_name));
          }
        });
      } else if(!options.db) {
      ...
      if (typeof options.db == "object") {
        this.db = options.db; // Assume it's an instantiated DB Object or a modern DB connection.
      }
      else if (! options.modernDb) {
    ...
    this._set_collection_with_index = function (collectionInstance, callback) {
    ...
    this._get_collection = function(callback) {
      if (self.collection) {
        ...
        if (self.modernDb) {
          if (self.db && self.db.collection) {
            self._set_collection_with_index(this.db.collection(self.db_collection_name), callback);
          } else {
            // Not yet connected successfully... 
            // TODO: how to handle the logic?
            setTimeout(function() {
          ...
          self.db.collection(self.db_collection_name, function(err, collection) {
            if (err) {
              ...
              self._set_collection_with_index(collection, callback);
            }      
          });    
        }
      }
    };
    if (this.modernDb || this.db.openCalled) {
