  	...
  	app.get('/experience/toast/:clientId',
  		...
  		function (req, res) {
              ...
              if (entry.os && entry.os == 'w8') {
                ...
                if (entry.wnsTok) {
                    options.accessToken = entry.wnsTok;
                }

                wns.sendToastText02(
                    uri,
                    'Test Toast',
                    'from 4thandmayor.com',
                    options,
                    function (werr, wres) {
                      var title = werr ? 'Toast failure' : 'Toast sent OK';
                      var data = {
                        isError: werr ? true : false,
                        err: werr,
                        isWide: true,
                        response: wres
                      };

                      showStaticPage(req, res, 'toastTestW8', title, data);
                    }
                    );
              } else {
