...
function initialize(config, callback) {
    ...
    if (Context.environment.isDevelopment) {
        Context.winston.info('In development mode, Winston will not log to the Azure Table Service.');
    } else {
        var AzureTableService = require('./winston-azuretableservice').AzureTableService;
        Context.winston.add(winston.transports.AzureTableService, {
              account: config.azure.storageaccount
            , accessKey: config.azure.storagekey
        //  , table: 'log' // Name of the table to use.
        //  , createTableIfNotExists: true // Whether to attempt to create the table if it does not exist.
        //  , partition: 'party' // Name of the partition to use.
            , secure: true // Use HTTPS. This is true by default.
        });
    }
