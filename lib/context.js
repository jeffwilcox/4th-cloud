...
function initialize(config, role, mode, callback) {
    ...
    if (Context.environment.isDevelopment || config.hosting.windowsAzure) {
...
function initializeMongo(context, callback) { // (err)
    // Immediately start the web site up.
    callback(undefined);

    // Begin connecting to Mongo DB.
    ...
    mongoClient.open(function(err, p_client) {
        mongoClient.authenticate(mongoCredentials.user, mongoCredentials.password, function(err, p_client) {
            if(err) {
                // callback(err);
                // TODO: REPORTING and DIAGNOSTICS: Very fatal.
            } else {
