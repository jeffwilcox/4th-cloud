...
module.exports = function(ctx) {
	...
	function executeTask(finalTaskCallback) {
		...
		this.clientResults = {
			...
			// TODO: REMOVE sentToast code ?
			sentToast: false // this is more of a LEGACY thing. // TODO: REMOVE
		};
	    ...
	    this.sendPushNotification = function (pushUri, instance, thisCallback) {
		    instance.send(pushUri, function (err1, res1) {
		        ...
		        if (err1) {
		            ...
		            // thisTask.clientResults.log('The ' + resultObject.pushType + ' send failed.');
		            if (err1.shouldDeleteChannel === true) {
		                ...
		                // thisTask.clientResults.log('The subscription channel should not be tried again for ' + err1.minutesToDelay + ' minutes.');
		        ...
		        if (resultObject.deviceConnectionStatus) {
		...
		function (err, results) {
			/*if (context.environment && context.environment.isDevelopment) {
				context.logger.info('Completed clientWorkerTask processing of ' + thisTask.uri + (err ? ' with errors' : ' ok'));
			}*/
			...
			if (clientErrors.minutesUntilNextTry > 0) {
				if (thisTask.storage.pollFrequency && thisTask.storage.pollFrequency > clientErrors.minutesUntilNextTry) {
					...
					// hadErrors = true;
            		// clientResults.log('Delaying the next client poll ' + clientErrors.minutesUntilNextTry + ' minutes.');
