...
module.exports = function (task) {
	...
	return function (callback) {        
        ...
        if (task.storage && !task.storage.selfCheckin) {
            callback(null, null); // no self data.
            return;
        }
        ...
        var self = task.storage.selfCheckin.user;
                ...
                if (err) {
                    ...
                    if (res && res.user) {
                        ...
                        context.mongo.collections.friends.findAndModify(
                            { userid: self.id, friendid: friendid }, // query
                            [['_id', 'asc']], // sort
                            {$set: friendEntry },
                            {
                                'new': true,
