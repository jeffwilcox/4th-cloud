...
module.exports = function (task) {
	return function (callback) {
        ...
        function sendFriendCheckinToast(friendName, venueName, venueId, checkinId, callback_) {
            var text = venueName;
            ...
            task.sendPushNotification(task.uri, toast, function (ce, cr) {
                if (!ce) {
                    clientResults.sentToast = true;
                    console.log('Sent a toast!');
		...
		if (task.storage.selfCheckin &&
            task.storage.selfCheckin.user && 
            task.storage.selfCheckin.user.id &&
            task.storage.pendingFriendToasts && 
            task.storage.pendingFriendToasts.length > 0) {
            
            var myId = task.storage.selfCheckin.user.id;
            ...
            context.mongo.collections.friends.findAndModify(                
                ...
                function(err, obj) {
                    if(err) {
                        ...
                        // Send the toast.
                        sendFriendCheckinToast(friendName, checkinVenue.name, checkinVenue.id, checkinId, callback);
                    }
                });
        } else {
            callback(null, null);
