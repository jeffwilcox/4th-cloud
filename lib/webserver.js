...
function downloadLocalImage(userId, photoUri, squareWidth, callback) {
    ...
    // console.log('TEMP: ' + localMiniPhotoPath);
...
exports.initialize = function(context) {
    var app = express.createServer(); // var app = module.exports = express.createServer();
...
//    var// MINI_PHOTOS_DIRECTORY = context.environment.isWindows === true ? "c:\\temp\\49photos\\" : '/tmp/49photos/';
    ...
    var jeffUserId = '20057'; // my foursquare account number.
    ...
    app.configure(function() {
        ...
        app.use(express.session({ 
            ...
            store: new MongoStore({
                ...
                host : configuration.mongo.server,
                port : configuration.mongo.port,
                username: configuration.mongo.user,
...
//    app.get('/experience', ensureAuthenticated, function(req, res){
  //    res.render('experience', 
    //    { 
      //      title : 'My 4th & Mayor Experience',
        //    user: req.user,
          //  isWide: true });
//    });
                    ...
                    for(var item in document) {
                        ...
                        if(pushutil.isValidUri(client)) {
                            ...
                            context.mongo.collections.history.find(
                                ...
                                if(wasError) {
                                    s += getXamlTextBlock('Hello, ' + nameEntry + '. Unfortunately push notifications are still in testing at this time.', true);
                                }
    ...
    app.post('/v1/connect', function (req, res) {
        // https://www.4thandmayor.com/v1/connect/?uri=http://sn1.notify.live.net/throttledthirdparty/01.00/AAFc96v0NuarQJSqSQ9KMwxgAgAAAAADAQAAAAQUZm52OjIzOEQ2NDJDRkI5MEVFMEQ&oat=IC0QWIWQN2GK0WLDXDTJQPF5ZYWCF034AQOJVQ5CLHMXQOFL&u=20057&apv=7.0&av=3.0.0.0&cc=0&uc=7&mfg=Microsoft+XDeviceEmulator&osv=7.10.7720
        ...
        // Windows 8 was the first app that is sending this always. [w8]
        var os = req.param('os');
        if (os !== undefined)
        {
            document.os = os;
        ...
        if(context.mongo.collections.clients !== null) {
            context.mongo.collections.clients.findAndModify(
                ...
                function(err, obj) {
                    ...
                    console.log('push notifications connection for: ' + extra);

                    res.send('ok');
                });
        }
        else {
            console.log('no mongoClientsCollection');
    ...
    app.get('/v1/push/status.html', function (req, res) {
        ...
        console.log('status.html for user');

        res.contentType('text/html');
        res.send('<html><body>Push notifications are almost out of beta! Tweet to @4thandmayor for assistance.</body></html>');
    });
    ...
    app.get('/tile.php', function (req, res) {
        ...
        if(original) {
            console.log('live tile resize processed');
        ...
        for(var i = 1; i < 4; i++) {
            ...
            if(nameLine) {
                ...
                if(userId && score) {
                    ...
                    if(path.existsSync(context.configuration.path.temporaryPhotosDirectory + userId + '.png')) {
            ...
            if(leaderImagesToDownload.length > 0) {
                ...
                downloadLocalImage(toGet.u, 
                    ...
                    if(err) {
                        ...
                        console.log('Could not save a mini image from the web.');
                    } else {
                        console.log('Downloaded a small image locally for ' + toGet.u);
                        if(path.existsSync(context.configuration.path.temporaryPhotosDirectory + toGet.u + '.png')) {
                            leaders.push(toGet);
                        }
                    }
